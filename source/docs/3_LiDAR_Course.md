...
Rotating LiDAR: Rotating LiDAR performs full horizontal scanning by rotating the emitter or scanning mechanism. It usually offers high scanning speed and high measurement accuracy, and is widely used in autonomous driving, 3D environment modeling, and map construction.

Solid-State LiDAR: Solid-state LiDAR uses a solid-state laser emitter and does not require rotating parts. It is generally more compact, lighter, and lower in power consumption. It is suitable for mobile devices, drones, robots, and similar applications.

Mechanical LiDAR: Mechanical LiDAR uses mechanical components such as rotating mirrors or rotating prisms to scan the laser beam. It usually provides longer measurement distance and higher measurement accuracy, but the scanning speed is relatively lower. It is widely used in terrain surveying, building scanning, and navigation.

Phase-Modulated LiDAR: Phase-modulated LiDAR measures the distance between the target object and the LiDAR by changing the phase of the laser beam. It usually provides high measurement accuracy and a large measurement range, and is widely used in map construction, surveying, and industrial applications.

Flash LiDAR: Flash LiDAR uses a short, high-power laser pulse to illuminate the entire scene at one time, and then captures the reflected light signal through a receiver array. It offers the advantages of high speed and high resolution, and is suitable for rapid scene capture and motion tracking.
...
TOF can be understood with the figure below. The LiDAR first projects light onto the object. The object reflects the light directly back to the LiDAR. The LiDAR calculates the return time of the light, then multiplies that time by the speed of light to obtain the distance between the object and the LiDAR.
...
This feature can be enabled in two ways. The first is through the mobile app. The second is through commands after a remote connection to the system.

For the mobile app method, refer to [1.5 App Installation and Connection]().

1. Start the robot and connect it through NoMachine. For remote connection instructions, refer to [1.7.2 AP Mode Connection Steps]().
...
5. Open another terminal, enter the following command, and press **Enter** to enter the LiDAR feature.
...
8. To close this feature, press **Ctrl+C** directly in the terminal window used in Step 4 or Step 5.

## 3.4 LiDAR Following

The robot detects the distance between the object directly in front of it and the robot body. When the object gets too close to the front of the robot, the robot performs a turning maneuver to avoid the obstacle.
...
This function can only be used with the Mecanum chassis version.
